...
import { GeoDataModelI } from '../models';
...
export class MapComponent implements OnInit, OnDestroy {
  ...
  private accessToken = 'pk.eyJ1IjoibWFrZW5vaXplciIsImEiOiJjazN1YWwzZmYwYXJ3M25xOG9jcjJ0bjZlIn0.ylHg0tCIXrvWRuB9ttVpOw';
  ...
  readonly colorOfLayer: string = '#6f4cfc';

  constructor(private dataService: DataService) {
    this.initAccidentsDataSubscription = this.dataService.initAccidentsData.subscribe((data: GeoDataModelI) => {
      this.addAccidentsData(data);
      localStorage.setItem('accidentsData', JSON.stringify(data));
    });
    this.accidentsDataSubscription = this.dataService.accidentsData.subscribe((data: GeoDataModelI) => this.updateAccidentsData(data));
  }
  ...
  public initViewOfMap() {
    this.dataService.viewOfMap.subscribe(view => {
      this.style = view.url;
      this.updateMap();
    });
  }
  ...
  private updateMap() {
    this.addMap();
    const data = JSON.parse(localStorage.getItem('accidentsData') as string);
    this.addAccidentsData(data);
  }
  ...
  private addAccidentsData(data: GeoDataModelI): void {
    this.map.on('load', () => {
  ...
  private updateAccidentsData(data: GeoDataModelI): void {
